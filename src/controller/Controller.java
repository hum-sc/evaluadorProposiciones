...
public class Controller {
    ...
    private Power createPower(Operation left){
    	Power power = null;
        Operation right = selectRight();
        power = new Power(left, right);
        return power;
    }
    ...
    private Operation createType(Operation left, int option) {
        ...
        switch (option) {
            ...
            case 5:
                ...
                break;
            case 6:
                operationLocal = createPower(left);
                break;
            default:
                break;
        }
    ...
    private Operation selectLeft() {
        ...
        try {
            ...
            if(option >= 7 && option <= 7+maxLetters-65){
                operation += options[option];
    ...
    private Operation selectRight(){
        ...
        try {
            ...
            if(option >= 6 && option <= 7+maxLetters-65){
                operation += options[option];
    ...
    private Operation selectType(Operation left) {
        view.showInputOptions(operation+instruccion, new int[]{0,1,2,5,6});
        int option;
        Operation operationLocal = null;
        try {
            option = view.getOption();
    ...
    private Operation operationEnded(Operation operation){
        view.showInputOptions(this.operation+instruccion, new int[]{0,1,2,5,6,4});
        int option;
